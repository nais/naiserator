...
func TestPreparePostgresEngineResolution(t *testing.T) {
	...
	testCases := []struct {
		name       string
		status     *postgresapi.PostgresStatus
		wantEngine string
		wantErr    string
	}{
		{
			name:       "uses engine when present",
			status:     &postgresapi.PostgresStatus{Engine: postgres.EngineCNPG},
		...
		},
		{
			...
			status:  &postgresapi.PostgresStatus{Engine: "unknown"},
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := postgresapi.AddToScheme(scheme); err != nil {
				t.Fatalf("add legacy Postgres API to scheme: %v", err)
			}

			pg := &postgresapi.Postgres{
				ObjectMeta: metav1.ObjectMeta{Name: "my-pg-cluster", Namespace: "team-a"},
				Status:     tt.status,
			}

			cl := fake.NewClientBuilder().WithScheme(scheme).WithRuntimeObjects(pg).Build()
