...
func TestGetDeployment(t *testing.T) {
	...
	t.Run("Test deployment with vault", test.EnvWrapper(map[string]string{
		...
		t.Run("user settings is applied", func(t *testing.T) {
			...
			assert.Equal(t, app.Spec.Healthcheck.Liveness.Path, appContainer.LivenessProbe.HTTPGet.Path)
			assert.Equal(t, int32(app.Spec.Healthcheck.Liveness.PeriodSeconds), appContainer.LivenessProbe.PeriodSeconds)
			assert.Equal(t, int32(app.Spec.Healthcheck.Liveness.Timeout), appContainer.LivenessProbe.TimeoutSeconds)
			assert.Equal(t, int32(app.Spec.Healthcheck.Liveness.FailureThreshold), appContainer.LivenessProbe.FailureThreshold)
			assert.Equal(t, int32(app.Spec.Healthcheck.Liveness.InitialDelay), appContainer.LivenessProbe.InitialDelaySeconds)
			assert.Equal(t, nais.DefaultPortName, appContainer.ReadinessProbe.HTTPGet.Port.StrVal)
			assert.Equal(t, app.Spec.Healthcheck.Readiness.Path, appContainer.ReadinessProbe.HTTPGet.Path)
			assert.Equal(t, int32(app.Spec.Healthcheck.Readiness.PeriodSeconds), appContainer.ReadinessProbe.PeriodSeconds)
			assert.Equal(t, int32(app.Spec.Healthcheck.Readiness.Timeout), appContainer.ReadinessProbe.TimeoutSeconds)
			assert.Equal(t, int32(app.Spec.Healthcheck.Readiness.FailureThreshold), appContainer.ReadinessProbe.FailureThreshold)
			assert.Equal(t, int32(app.Spec.Healthcheck.Readiness.InitialDelay), appContainer.ReadinessProbe.InitialDelaySeconds)
