...
func (v *Registry) Routes(host string) []networking_istio_io_v1alpha3.HTTPRoute {
	routes := make([]networking_istio_io_v1alpha3.HTTPRoute, 0, len(v.routes))
	for parsedURL, route := range v.routes {
...
func (v *Registry) VirtualService(host string) networking_istio_io_v1alpha3.VirtualService {
	return networking_istio_io_v1alpha3.VirtualService{
		...
		Spec: networking_istio_io_v1alpha3.VirtualServiceSpec{
			Gateways: ResolveGateway(host, v.mappings),
			Hosts:    []string{host},
			HTTP:     v.Routes(host),
		},
	}
...
func (v *Registry) Add(app *nais_io_v1alpha1.Application) error {
